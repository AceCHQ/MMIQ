...
from utils.data_utils import load_yaml, construct_prompt, save_json, save_jsonl, process_single_sample
...
def run_model(args, samples, model, call_model_engine_fn=None, tokenizer=None, processor=None):
    ...
    with torch.no_grad():
        for sample in tqdm(samples):
            ...
            out_samples[sample['id']] = {"category":sample["category"], "response":response, "data_id":sample["data_id"]}
    return out_samples
...
def main():
    ...
    parser.add_argument('--data_path', type=str, default="MMIQ/MMIQ") # hf dataset path.
    ...
    save_jsonl(args.output_path, out_samples)
